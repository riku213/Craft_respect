...
class PreTrainDataset_v3(Dataset):
    ...
    def __init__(self,
                 ...
                 raster_cache_max_items=0,
                 version_tag='v1',
                 width_pkl_cache=True,
                 width_pkl_cache_prefix='gt_cache_v3',
                 width_cache_autosave=True):
        super().__init__()
        ...
        self.width_pkl_cache = width_pkl_cache
        self.width_pkl_cache_prefix = width_pkl_cache_prefix
        self.width_cache_autosave = width_cache_autosave
        ...
        # メモリ内ラスタLRU (簡易)
        self._raster_mem_cache = {}  # type: Dict[str, torch.Tensor]
        self._raster_mem_cache_access = []  # type: List[str]

        # 幅ごとのpklキャッシュ(画像ID -> Tensor)ロード
        self.width_cache_path = os.path.join(
            self.cache_dir, f"{self.width_pkl_cache_prefix}_w{self.target_width}.pkl") if self.width_pkl_cache else None
        self._width_gt_cache = {}  # type: Dict[str, torch.Tensor]
        if self.width_pkl_cache and self.width_cache_path and os.path.exists(self.width_cache_path):
            try:
                with open(self.width_cache_path, 'rb') as f:
                    loaded = pickle.load(f)
                for k, v in loaded.items():
                    if isinstance(v, np.ndarray):
                        loaded[k] = torch.from_numpy(v)
                self._width_gt_cache = loaded
                print(f"[width-cache] loaded {len(self._width_gt_cache)} entries from {self.width_cache_path}")
            except Exception as e:
                print(f"[width-cache] failed to load ({e}) -> start empty")

    # ========================= 内部: 幅キャッシュ保存 =========================
    def _save_width_cache(self):
        if not self.width_pkl_cache or not self.width_cache_path:
            return
        try:
            # CPUへ移してから保存 (GPU Tensor 防止)
            save_dic = {}
            for k, v in self._width_gt_cache.items():
                if isinstance(v, torch.Tensor):
                    save_dic[k] = v.cpu()
                else:
                    save_dic[k] = v
            with open(self.width_cache_path, 'wb') as f:
                pickle.dump(save_dic, f)
        except Exception as e:
            print(f"[width-cache] save failed: {e}")
    ...
    def __getitem__(self, index):
        ...
        # GT 取得 (優先順位: 幅pklキャッシュ -> precomputed -> 動的生成)
        if self.width_pkl_cache and image_id in self._width_gt_cache:
            tensor_gt = self._width_gt_cache[image_id]
        elif self.precomputed_gt and image_id in self.precomputed_gt:
            ...
            # pkl幅キャッシュへ登録
            if self.width_pkl_cache:
                self._width_gt_cache[image_id] = tensor_gt
                # 初回アクセス時にファイル作成/以降更新
                if self.width_cache_autosave:
                    self._save_width_cache()
