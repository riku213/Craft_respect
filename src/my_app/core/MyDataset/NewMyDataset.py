...
class PreTrainDataset_new(Dataset):
    ...
    def return_tensor_gt_optimized(self, gt_info_dic, image):
        ...
        for channel_name in channel_names:
            # 新しいキャンバスを作成
            canvas = torch.zeros(h, w, dtype=torch.float32, device=self.device)
            ...
            if channel_name in gt_info_dic and gt_info_dic[channel_name]:
                ...
                # ガウスマップを生成
                canvas = self.design_gaussian_map_gpu(
                    canvas,
                    scaled_points,
                    w, h
                )
            ...
            # CPUに転送して保存
            canvas_tensors.append(canvas.cpu())
    ...
    def add_perspective_gaussian_gpu(self, canvas, src_points, canvas_width, canvas_height):
        ...
        width = int(max(width, 10)) + 1  # 最小サイズを保証
        height = int(max(height, 10)) + 1
        
        # 正規化されたガウス分布を生成（振幅1.0）
        gaussian = self.create_gaussian_kernel_gpu(width, height)
        
        # 透視変換のための点を設定（直接NumPy配列として）
        dst_np = np.array([
            [0, 0],
            [width - 1, 0],
            [width - 1, height - 1],
        ...
        # try:
        # 透視変換行列の計算（CPUで実行）
        src_np = src_points.cpu().numpy().astype(np.float32)
        matrix = cv2.getPerspectiveTransform(dst_np, src_np)
        matrix_tensor = torch.from_numpy(matrix).to(dtype=torch.float32, device=self.device)
        
        # 変換をGPU上で実行
        transformed_gaussian = self.warp_perspective_gpu(
            gaussian, matrix_tensor, canvas_width, canvas_height
        )
        
        # old_MyDatasetと同様に加算で合成
        canvas = canvas + transformed_gaussian
    ...
    def create_gaussian_kernel_gpu(self, width, height):
        ...
        # ガウス分布のパラメータ（より鋭いピークを生成）
        sigma_x = width / 5.0  # old_MyDatasetと同じ値に変更
        sigma_y = height / 5.0
    ...
    def warp_perspective_gpu(self, image_tensor, matrix, output_width, output_height):
        """GPU上で透視変換を実行"""
        # matrixが既にテンソルであることを確認
        matrix_tensor = matrix if isinstance(matrix, torch.Tensor) else torch.from_numpy(matrix).float().to(self.device)
    ...
    def create_transformation_grid(self, matrix, height, width):
        ...
        # 同次座標に変換（float32型を明示的に指定）
        ones = torch.ones_like(x_grid, dtype=torch.float32)
