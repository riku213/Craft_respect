...
def _build_mask_cpu(gray: np.ndarray) -> np.ndarray:
    blurred = cv2.GaussianBlur(gray, (5, 5), 0)
    _, mask = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    kernel = np.ones((3, 3), np.uint8)
    return cv2.dilate(mask, kernel, iterations=1)

def _build_mask_gpu(gray: np.ndarray) -> np.ndarray:
    # OpenCV が CUDA 有効ビルドである必要あり
    g = cv2.cuda_GpuMat()
    g.upload(gray)

    # GaussianBlur
    gf = cv2.cuda.createGaussianFilter(cv2.CV_8U, cv2.CV_8U, (5, 5), 0)
    g_blur = gf.apply(g)

    # 縮小してCPUでOTSU閾値を取得
    h, w = gray.shape[:2]
    ds = (max(1, w // 4), max(1, h // 4))
    g_small = cv2.cuda.resize(g_blur, ds)
    blurred_small = g_small.download()
    thr_val, _ = cv2.threshold(blurred_small, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    # 取得した閾値でGPU側で二値化（反転）
    _, g_mask = cv2.cuda.threshold(g_blur, thr_val, 255, cv2.THRESH_BINARY_INV)

    # 膨張
    kernel = np.ones((3, 3), np.uint8)
    morph = cv2.cuda.createMorphologyFilter(cv2.MORPH_DILATE, cv2.CV_8U, kernel)
    g_mask = morph.apply(g_mask)

    return g_mask.download()

def remove_ink(image: np.ndarray, show_flag=False, inpaint_radius=20) -> np.ndarray:
    gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    use_cuda = hasattr(cv2, "cuda") and cv2.cuda.getCudaEnabledDeviceCount() > 0
    try:
        dilated_mask = _build_mask_gpu(gray) if use_cuda else _build_mask_cpu(gray)
    except Exception:
        # 例外時はCPUへフォールバック
        dilated_mask = _build_mask_cpu(gray)
    # inpaint はCPUのみ
    return cv2.inpaint(image, dilated_mask, inpaint_radius, cv2.INPAINT_TELEA)
...
def calc_center_of_gravity(gray_image):
    ...
    # return int(median_x)
        ...
        # 重み=画素値のx方向合計。オーバーフローを避けて累積和を取る
    ink_density_x = np.sum(gray_image, axis=0, dtype=np.uint64)
    total = int(ink_density_x.sum())
    if total == 0:
        # インク無しなら中央
        return gray_image.shape[1] // 2

    half = (total + 1) // 2
    cum = np.cumsum(ink_density_x, dtype=np.uint64)
    median_x = int(np.searchsorted(cum, half, side='left'))
    return median_x
...
def procedure_for_one_image(image, file_path=None,json_gt = None, show_flag=False, lock=None,):
    ...
    #                  lock=lock)
    save_gt_shard(file_id=file_id, 
                  data = new_json_data, 
                  shards_dir=GT_JSON_SHARDS_DIR)
...
def save_gt_shard(file_id: str, data: dict, shards_dir: str):
    os.makedirs(shards_dir, exist_ok=True)
    shard_path = os.path.join(shards_dir, f'{file_id}.json')
    tmp = shard_path + '.tmp'
    with open(tmp, 'w', encoding='utf-8') as f:
...
def build_processed_id_set(shards_dir: str, gt_json_path: str = None, image_dir: str = None) -> set:
    processed = set()
    # シャード
    if os.path.isdir(shards_dir):
        for e in os.scandir(shards_dir):
            if e.is_file() and e.name.endswith('.json'):
                processed.add(e.name[:-5])  # remove ".json"
    # 既存のgt_json（過去にマージ済み分）
    if gt_json_path and os.path.exists(gt_json_path):
        try:
            with open(gt_json_path, 'r', encoding='utf-8') as f:
                base = json.load(f)
            if isinstance(base, dict) and isinstance(base.get('files'), dict):
                processed.update(base['files'].keys())
        except Exception:
            pass
    # 既に吐き出したJPG
    if image_dir and os.path.isdir(image_dir):
        for e in os.scandir(image_dir):
            if e.is_file() and e.name.lower().endswith('.jpg'):
                processed.add(os.path.splitext(e.name)[0])
    return processed


def merge_gt_shards(shards_dir: str, output_path: str, backup: bool = True, clean: bool = False) -> int:
    ...
    if os.path.exists(output_path):
        with open(output_path, 'r', encoding='utf-8') as f:
            base = json.load(f)
        if 'files' not in base or not isinstance(base['files'], dict):
            base['files'] = {}
    else:
        base = {'files': {}}
    ...
    for e in os.scandir(shards_dir):
        if not e.is_file() or not e.name.endswith('.json'):
            continue
        file_id = e.name[:-5]  # remove ".json"
        with open(e.path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        base['files'][file_id] = data
        merged += 1

    if backup and os.path.exists(output_path):
        backup_path = output_path.split('.json')[0] + '_backup.json'
        _write_json_atomic(backup_path, base)

    _write_json_atomic(output_path, base)
...
if __name__ == "__main__":
    ...
    with Manager() as manager:
        ...
        with ProcessPoolExecutor(max_workers=7) as executor:
            ...
            # json_gt = load_GT_json(GT_JSON_PATH)
            json_gt = None
            processed_ids = build_processed_id_set(
                GT_JSON_SHARDS_DIR, 
            ...
            with tqdm(total = len(futures), ncols=80) as pbar:
                for f in as_completed(futures):
                    try:
                        ...
                        pbar.update(1)
            try:
                merged = merge_gt_shards(GT_JSON_SHARDS_DIR, GT_JSON_PATH, backup=True, clean=False)
                print(f"[merge] merged {merged} items into {GT_JSON_PATH}")
            except Exception as e:
